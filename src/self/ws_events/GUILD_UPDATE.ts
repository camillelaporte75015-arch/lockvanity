...
export { httpAgent } from "./GUILD_UPDATE";
...
async function patchVanity(
   ...
   for (let i = 0; i < ATTEMPTS; i++) {
       try {
           ...
           // Erreur réseau — on retente immédiatement
       }
   }
...
async function notifyMe(
   ...
   // Envoi dans le groupe broadcast
   client.broadcast(alertMessage);

   // Envoi dans le salon de logs si configuré
   const logsChannelId = await client.db.get(`logs.${guildId}`);
   if (!logsChannelId) return;

   const logsChannel = guild.channels.cache.get(logsChannelId) as any;
   if (!logsChannel) return;

   const pingEnabled = await client.db.get(`logping.${guildId}`);
   const prefix = pingEnabled ? "@everyone\n" : "";

   logsChannel.send(prefix + alertMessage).catch(() => {});
}
